...
static pair<double, vector<int>> compute1TreeOnCandidates(const vector<double> &P, vector <pair<int, int>> &edges)
{
    sort(edges.begin(), edges.end(), [&P](pair<int, int> lhs, pair<int, int> rhs) {
        return euclid(lhs.first, lhs.second) + P[lhs.first] + P[lhs.second] <
               euclid(rhs.first, rhs.second) + P[rhs.first] + P[rhs.second];
    });
    vector<int> newD(n, -2);
    vector<int> boundTo(n); // parent dla liści
    double weight = 0;
...
static pair<double, vector<int>> compute1TreeOnEuclidean(const vector<double> &P)
{
